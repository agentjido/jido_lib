...
#
# Usage:
#   ./test_opencode_sprite.sh https://github.com/OWNER/REPO/issues/123
#
# Options:
...
#     OPENCODE_MODEL (optional, defaults to zai_custom/glm-4.5-air)
#
set -euo pipefail

PROVIDER="opencode"
PROVIDER_LABEL="OpenCode"
SCRIPT_NAME="test_opencode_sprite.sh"
...
sprite_exec() {
  ...
  for var in \
    ...
    AMP_API_KEY AMP_URL \
    OPENAI_API_KEY \
    ZAI_API_KEY ZAI_BASE_URL OPENCODE_MODEL \
...
provider_binary() {
  case "$PROVIDER" in
    claude|amp|codex|gemini|opencode) echo "$PROVIDER" ;;
    *) echo "$PROVIDER" ;;
  esac
}
...
provider_step_9_title() {
  case "$PROVIDER" in
    ...
    opencode) echo "Step 9: Provision OpenCode CLI for JSON streaming (non-interactive)" ;;
...
provider_step_10_title() {
  case "$PROVIDER" in
    ...
    opencode) echo "Step 10: OpenCode Code Change (JSON)" ;;
  ...
  case "$PROVIDER" in
    ...
    opencode)
      if [[ -z "${ZAI_API_KEY:-}" ]]; then
        fail "OpenCode requires ZAI_API_KEY for non-interactive Z.AI provider setup"
        exit 1
      fi
      if [[ -z "${ZAI_BASE_URL:-}" ]]; then
        ZAI_BASE_URL="https://api.z.ai/api/anthropic"
        export ZAI_BASE_URL
        warn "ZAI_BASE_URL not set; defaulting to ${ZAI_BASE_URL}"
      fi
      if [[ -z "${OPENCODE_MODEL:-}" ]]; then
        OPENCODE_MODEL="zai_custom/glm-4.5-air"
        export OPENCODE_MODEL
        warn "OPENCODE_MODEL not set; defaulting to ${OPENCODE_MODEL}"
      fi
      ;;
    *)
...
ensure_provider_cli_in_sprite() {
  ...
  case "$PROVIDER" in
    ...
    opencode)
      local opencode_help
      local opencode_run_help
      opencode_help=$(sprite_exec "opencode --help 2>&1")
      opencode_run_help=$(sprite_exec "opencode run --help 2>&1")
      echo "$opencode_help" | grep -q -- "run" || {
        fail "opencode --help missing run command"; exit 1
      }
      echo "$opencode_run_help" | grep -q -- "--format" || {
        fail "opencode run --help missing --format"; exit 1
      }
      echo "$opencode_run_help" | grep -q -- "json" || {
        fail "opencode run --help missing json output support"; exit 1
      }
      ok "OpenCode JSON output flags detected"
      ;;
  esac
}
...
configure_opencode_runtime() {
  ...
  env_check=$(sprite_exec 'if [ -n "${ZAI_API_KEY:-}" ] && [ -n "${ZAI_BASE_URL:-}" ] && [ -n "${OPENCODE_MODEL:-}" ]; then echo present; else echo missing; fi')
  if echo "$env_check" | grep -q "missing"; then
    fail "OpenCode requires ZAI_API_KEY, ZAI_BASE_URL, and OPENCODE_MODEL visible in sprite"
    exit 1
  fi
  ok "OpenCode Z.AI env config visible inside sprite"

  info "Writing opencode.json for non-interactive Z.AI provider config..."
  sprite_exec "cat > opencode.json << 'JIDO_OPENCODE_CONFIG_EOF'
{
...
JIDO_OPENCODE_CONFIG_EOF" "$REPO_DIR" || {
    fail "Failed to write opencode.json inside sprite repo"
    exit 1
  }
  ok "opencode.json configured in repository root"

  model_check=$(sprite_exec "opencode models zai_custom 2>&1" "$REPO_DIR") || {
    fail "OpenCode could not resolve custom zai_custom provider"
    exit 1
  }
  echo "$model_check" | grep -q "zai_custom/" || {
    fail "OpenCode custom provider check failed: $(shorten "$model_check" 240)"
    exit 1
  }
  ok "OpenCode custom Z.AI provider loaded"
}
...
provider_smoke_command() {
  ...
  case "$PROVIDER" in
    ...
    gemini)
      ...
      ;;
    opencode)
      echo "if command -v timeout >/dev/null 2>&1; then timeout 120 opencode run --model \${OPENCODE_MODEL:-zai_custom/glm-4.5-air} --format json \\\"\$(cat ${pf_q})\\\"; else opencode run --model \${OPENCODE_MODEL:-zai_custom/glm-4.5-air} --format json \\\"\$(cat ${pf_q})\\\"; fi"
      ;;
    *)
...
provider_change_command() {
  ...
  case "$PROVIDER" in
    ...
    gemini)
      ...
      ;;
    opencode)
      echo "if command -v timeout >/dev/null 2>&1; then timeout 180 opencode run --model \${OPENCODE_MODEL:-zai_custom/glm-4.5-air} --format json \\\"\$(cat ${pf_q})\\\"; else opencode run --model \${OPENCODE_MODEL:-zai_custom/glm-4.5-air} --format json \\\"\$(cat ${pf_q})\\\"; fi"
      ;;
    *)
...
provider_smoke_prompt() {
  case "$PROVIDER" in
    ...
    opencode) echo "Return exactly one line: JIDO_OPENCODE_SMOKE_OK. Do not use tools. Do not ask questions." ;;
...
echo "   9. ${PROVIDER_LABEL} JSON smoke prompt"
echo "  10. ${PROVIDER_LABEL} JSON code change"
