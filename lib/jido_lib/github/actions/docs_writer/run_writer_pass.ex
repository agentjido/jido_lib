defmodule Jido.Lib.Github.Actions.DocsWriter.RunWriterPass do
  ...
  defp writer_prompt(params, 1) do
    ...
    {updated, _directives} = MyApp.Example.cmd(agent, [SomeAction])
    ...
    8. NimbleOptions: ONLY `type:`, `default:`, `required:` — NEVER `doc:`
  ...
  defp writer_prompt(params, 2) do
    ...
    6. Call `YourModule.cmd(agent, actions)` — the cmd/2 is on YOUR module, not Jido.Agent.
    7. `cmd/2` returns `{agent, directives}` 2-tuple — NEVER `{:ok, agent, directives}` (MatchError!).
    8. Define ALL variables before referencing them (no undefined variable errors).
    9. The LAST ```elixir block MUST contain `DemoAgent = YourAgentModule`.
    10. Every ```elixir block must compile and run independently when concatenated.
    """
    |> String.trim()
  end
