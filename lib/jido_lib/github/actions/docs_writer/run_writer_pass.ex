defmodule Jido.Lib.Github.Actions.DocsWriter.RunWriterPass do
  ...
  defp writer_prompt(params, 1) do
      ...
      if has_grounded_context do
        ...
        - CRITICAL API PATTERN: `use Jido.Agent` generates `new/0`, `cmd/2`, `set/2` on
          YOUR module. So the correct calls are:
            ...
            {:ok, agent, dirs} = YourModule.cmd(agent, [Action])  # NOT Jido.Agent.cmd(...)
        """
      else
        ""
      end
    ...
    # Run commands via the module's own cmd/2
    {:ok, updated, _directives} = MyApp.Example.cmd(agent, [SomeAction])
    ...
    7. NimbleOptions: ONLY `type:`, `default:`, `required:` — NEVER `doc:`
  ...
  defp writer_prompt(params, 2) do
    ...
    5. Call `YourModule.new()` — NEVER `Jido.Agent.new(YourModule)` (FunctionClauseError!).
    6. Call `YourModule.cmd(agent, actions)` — the cmd/2 is on YOUR module, not Jido.Agent.
    7. Define ALL variables before referencing them (no undefined variable errors).
    8. The LAST ```elixir block MUST contain `DemoAgent = YourAgentModule`.
    9. Every ```elixir block must compile and run independently when concatenated.
    """
    |> String.trim()
  end
